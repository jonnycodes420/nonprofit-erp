...
// it fixtures; both get the same answer. It also holds the two sentences
// Finance puts beside its money-in figures (cash on hand, the Stripe balance),
// so the screen and tests/fix1-finance.test.js read one string.
...
// ── The two sentences beside Finance's money-in figures ────────────────────
// Every number on a screen has its one defining sentence. The walk found
// "Available $0" beside "Cash on hand $400.4k" with nothing saying why; the
// two are different kinds of number, and each now says what it is.
export const CASH_ON_HAND_SENTENCE =
  "Everything your ledger has taken in, minus everything it has paid out, since you started using Steward. It is the ledger's figure, not a bank balance and not Stripe's.";

export function stripeBalanceSentence({ availableCents = 0, pendingCents = 0, cashOnHandText = "" } = {}) {
  if (int(availableCents) === 0 && int(pendingCents) === 0) {
    return "Stripe shows $0 because it has already paid everything it held out to your bank. " +
      `Cash on hand${cashOnHandText ? ` (${cashOnHandText})` : ""} is your ledger's figure: every gift, cheque and ` +
      "payment Steward has recorded, minus what has gone out, wherever the money now sits.";
  }
  return "This is money Stripe is still holding for you. When it pays out it reaches your bank, " +
    "and each payout opens to the gifts, refunds and fees inside it.";
}
