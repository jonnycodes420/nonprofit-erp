...
export const MONEY_PATTERNS = [
  ...
  // FIX-1: RECORDING a gift she tells it about is no longer refused here —
  // it is PREPARED for a person to confirm (preparedGiftFromInstruction). The
  // agent still never records money; the absence of an executor is unchanged.
];
...
//
// So the model no longer writes a headline at all. It returns STEPS — each one
// a concrete action on a named row — and Steward COMPILES the headline from
// those steps (compilePlan below). The run then executes exactly the steps the
// plan showed, and nothing else: no second model call, no new actions.
...
export function validatePlan(plan, { authorization = AUTH_DRAFT } = {}) {
  ...
  for (const s of steps) {
    ...
    if (tool.needsHuman === "always") errors.push(`${s.tool} moves money and is never planned by the model`);
...
function clauseFor(tool, group, byId) {
  ...
  switch (tool) {
    case "record_gift": {
      if (n === 1) {
        const s = group[0];
        return `record a ${money(s.amountCents)} gift from ${who || "a giver you name"} once you confirm it`;
      }
...
export function describeStep(step, byId = new Map()) {
  ...
  switch (step.tool) {
    case "record_gift": return `Record a ${money(step.amountCents)} gift from ${p ? who : "a giver you name"}. Prepared for you to confirm.`;
    case "open_thread": return `Open a follow-up with ${who}${step.label ? `: ${step.label}` : ""}.`;
...
export function compilePlan(steps, { people = [], reads = null, withheld = 0 } = {}) {
  ...
  else summary = `Steward will ${clauses.slice(0, -1).join(", ")} and ${clauses[clauses.length - 1]}.`;
...
const GIFT_NEWS = /\b(got|received|receive|came in|arrived|gave|donated|sent (us|in)|mailed|dropped off|gift|donation|check|cheque)\b/;
const AMOUNT_RES = [
...
export function isGiftNews(text) { return GIFT_NEWS.test(String(text || "").toLowerCase()); }
