...
export function AnnualFund({data}) {
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
      <div style={{display:"flex",gap:6}}>
        {[currentYear-1,currentYear].map(y2=><button key={y2} onClick={()=>{setYear(y2);load(y2);}}
          style={{background:year===y2?"#0d5c3a":"transparent",border:year===y2?"none":"1px solid #374151",borderRadius:8,padding:"7px 14px",color:year===y2?"#fff":T.ink3,fontSize:12,fontWeight:year===y2?700:400,cursor:"pointer"}}>{y2}</button>)}
      </div>
    ...
    {editGoal&&isAdmin&&<Card style={{display:"flex",gap:10,alignItems:"flex-end"}}>
      ...
      <button onClick={saveGoal} style={{background:"#0d5c3a",border:"none",borderRadius:8,padding:"10px 16px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>Save</button>
    ...
    {loading?<div style={{color:T.ink3,fontSize:13,textAlign:"center",padding:60}}>Loading annual fund data…</div>:!fund?null:<>
      <Card>
        <div style={{display:"flex",alignItems:"center",gap:20,flexWrap:"wrap"}}>
          ...
          {fund.goal>0&&<div style={{flexShrink:0}}>
            <svg width="88" height="88" viewBox="0 0 88 88">
              ...
              <circle cx="44" cy="44" r="36" fill="none" stroke="#0d5c3a" strokeWidth="9"
        ...
        {fund.goal>0&&<div style={{marginTop:14,height:6,background:T.bg3,borderRadius:99}}>
          <div style={{height:"100%",width:`${Math.min(fund.goalPct,100)}%`,background:fund.goalPct>=100?"#0d5c3a":fund.goalPct>=60?"#f59e0b":"#ef4444",borderRadius:99,transition:"width 0.5s"}}/>
        </div>}
      </Card>

      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10}}>
        <MetricCard label="Total Gifts" value={fund.giftCount} sub={`avg ${fmt(fund.avgGift)}`} color="#0d5c3a"/>
        ...
        <MetricCard label="Retention Rate" value={`${fund.donors.retentionRate}%`} sub="vs prior year" color={fund.donors.retentionRate>=70?"#0d5c3a":fund.donors.retentionRate>=50?"#f59e0b":"#ef4444"}/>
        ...
        <div style={{display:"flex",alignItems:"flex-end",gap:4,height:140}}>
          {fund.monthly.map(m=>{
            ...
            return <div key={m.month} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              ...
              <div style={{width:"100%",height:h,background:m.raised>0?(T.green600||"#0d5c3a"):T.bg2,borderRadius:"4px 4px 0 0",minHeight:3}}/>
      ...
      <Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:14}}>
          ...
          <div>
            <div style={{fontSize:28,fontWeight:800,color:"#0d5c3a",fontFamily:"'DM Serif Display',serif"}}>{fund.donors.retained}</div>
        ...
        <div style={{height:6,background:T.bg3,borderRadius:99,display:"flex",overflow:"hidden"}}>
          <div style={{width:`${fund.donors.total>0?Math.round(fund.donors.retained/fund.donors.total*100):0}%`,background:"#0d5c3a"}}/>
          <div style={{flex:1,background:"#3b82f6"}}/>
        </div>
        <div style={{display:"flex",gap:12,marginTop:8}}>
          <div style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:T.ink3}}><div style={{width:10,height:10,background:"#0d5c3a",borderRadius:2}}/>Retained</div>
