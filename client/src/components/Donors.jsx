...
// The chips are the roles a person can carry; each tap is ONE write through
// PUT /people/:id/roles. A role that is on is a STATE (cream, emerald
// hairline), never a second action colour. Adding Volunteer puts them on the
// Volunteers roster that moment, on this same record.
//
// DONOR IS SET BY GIVING. While gifts are on file the Donor chip cannot be
// turned off, and it says why on hover — the reason is the SERVER's sentence
// (GET /people/:id), and the server refuses the removal too, so the chip and
// the rule cannot disagree.
const ROLE_CHIPS=[
  {key:"donor",label:"Donor"},
  {key:"volunteer",label:"Volunteer"},
  {key:"staff_board",label:"Staff and board"},
];
function RoleChips({donor,isReadOnly,onChanged}){
  ...
  const toggle=async(k)=>{
    ...
    if(k==="donor"&&!on&&lock.locked){setErr(lock.reason);return;}
    setBusy(k);setErr("");
    try{
      ...
      if(r&&Array.isArray(r.person_types)){
        ...
        setLock({locked:!!r.donorLocked,reason:r.donorLockedReason||null});
        onChanged&&onChanged(r.person_types);
      }
    }catch(e){setErr(errorMessage(e,"Could not change this person's role"));}
    setBusy(null);
  };
  return (
    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginTop:6}} data-testid="role-chips">
      {ROLE_CHIPS.map(c=>{
        const on=types.includes(c.key);
        const locked=c.key==="donor"&&on&&lock.locked;
        const title=locked?lock.reason:(on?`Remove ${c.label}`:`Add ${c.label}`);
        return (
          <button key={c.key} type="button" data-testid={"role-chip-"+c.key} aria-pressed={on}
            disabled={isReadOnly||busy===c.key} onClick={()=>toggle(c.key)} title={title}
            style={{fontSize:11,fontWeight:700,padding:"4px 11px",borderRadius:99,lineHeight:1.3,
      ...
      {err&&<span role="alert" style={{fontSize:11,color:T.ink2}}>{err}</span>}
    </div>
  );
}
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        // FIX-1 D — Donors shows donors. Volunteers live on their own hub,
        // staff and board under Settings, and search still finds anyone.
        qs.set("role","donor");
