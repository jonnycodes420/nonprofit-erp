...
// ledger's, not the bank's and not Stripe's, and it says so.
const CASH_ON_HAND_SENTENCE = "Everything your ledger has taken in, minus everything it has paid out, since you started using Steward.";
...
function MoneyInStrip({ onNavigate, onOpenPayout }) {
  ...
  return (
    <Card>
      ...
      <div data-testid="stripe-balance-sentence" style={{ fontSize:12, color:T.ink2, marginTop:10, lineHeight:1.55 }}>
        {s.balanceSentence || "This is money Stripe is still holding. Once it pays out it reaches your bank, and each payout below opens to the gifts inside it."}
      </div>
      {payouts.length > 0 && (
...
function PayoutsView({ onNavigate, openId, onOpen }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/finance/payouts/${encodeURIComponent(openId)}`)
  ...
  return (
    <div data-testid="payout-detail"><Card>
      ...
      {d && <>
        ...
        <div data-testid="payout-sentence" style={{ fontSize:13, color:d.reconciled ? T.ink2 : T.terra700, marginTop:6, lineHeight:1.55,
          ...(d.reconciled ? {} : { background:T.terra100, border:"1px solid "+T.terra200, borderRadius:8, padding:"8px 10px" }) }}>
          {d.sentence}
        </div>
                  ...
                  <td style={{ padding:"8px" }}>
                    {r.donorId
                      ? <span {...interactive(() => onNavigate && onNavigate("donors", { selectDonorId: r.donorId }), { label: `Open ${r.donorName}` })}
                          style={{ color:T.greenMid, fontWeight:700 }}>{r.donorName}</span>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {subtab === "overview" && <>
        ...
        <MoneyInStrip onNavigate={onNavigate} onOpenPayout={id => { setOpenPayout(id); setSubtab("payouts"); }}/>
        <Card>
