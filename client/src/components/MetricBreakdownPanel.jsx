import { T, Spin, Modal } from "./shared";
...
export default function MetricBreakdownPanel({ open, onClose, title, explanation, total, totalLabel, totalCount, rows = [], loading = false, onSelectDonor }) {
  // BUILD-87 F.1 — the portal, the Escape handler and the focus round-trip
  // that used to live here are the shared Modal's job now. The reason they
  // existed is worth keeping written down: the Dashboard's root retained a
  // `transform: translateY(0)` from `.fade-in`'s fill-mode, which made it the
  // containing block for every position:fixed descendant and dropped this
  // panel at the vertical middle of the whole tall page. BUILD-87 fixed the
  // fill-mode too, so the trap is gone at both ends.
  if (!open) return null;
  // BUILD-87 F.1 — this panel already portalled and already knew about focus;
  // what it does NOT need to keep is its own copy of the backdrop, the scroll
  // lock and the Escape handler. Its header is richer than a title and a line
  // of prose, so it passes one through `header` and keeps it.
  return (
    <Modal onClose={onClose} width={640} ariaLabel={title || "Details"}
      backdrop="rgba(15,26,18,0.5)" blur={false} padding={0}
      dialogStyle={{ borderRadius: T.radiusLg, maxHeight: "82vh" }}
      header={(
