...
(async () => {
  ...
  ok("nothing in the note → the touch-type default",
     meetingPlain.source.from === "touch" && meetingPlain.label === shape.NEXT_STEP_DEFAULTS.meeting.label, meetingPlain);
  ...
  ok("a note-derived step carries no template follow-on chain",
  ...
  ok("the template step keeps its label and its follow-on",
     th2.next_step_label === shape.NEXT_STEP_DEFAULTS.meeting.label && !!th2.followon_type, th2.next_step_label);
