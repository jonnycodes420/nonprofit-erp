...
- **Known next cliffs (flagged in LOADTEST_REPORT.md, deliberately not fixed)**: `GET /donors` returns the entire org unpaginated — 21.7MB of JSON at 25k donors; the query is fine (~0.4s), the payload/DOM is the cliff, and pagination is invasive (Directory, Kanban, onboarding, exports all read it). Retention still JS-parses every gift row (~150ms per Home load at 200k gifts). Import recalc and autoEnroll enrollment checks are cheap-but-N+1. bcrypt serializes logins at ~10/s under burst (by design).
