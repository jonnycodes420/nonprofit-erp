...
## Project structure
- /client/src/App.jsx — AppShell + TABS + App, imports from components/. Header is wordmark-only (no icon-in-square logo — icon lives on the favicon only) and has no "Ask AI" button (removed, see Strategic pivot). **Per-tab width strategy (BUILD-06 Phase E, 2026-07-17)**: the `app-content` wrapper gives Home a centered ~1200px column (reading page) while every workspace tab (Donors/Grants/Communications/Reports/Settings + hidden legacy tabs) is fluid full-width with 32px side padding; the shell background matches the tab (`T.bgDeep` on Home so Dashboard's `dash-bleed` never leaves lighter gutters, `T.bg` elsewhere). Mobile is exempt — GlobalStyles' 768px `!important` rules override the inline widths. Grants Kanban columns are `flex:"1 1 260px"` (grow to fill wide screens); Donor Kanban keeps its deliberate count-proportional fr weights.
...
- **`DISABLE_RATE_LIMIT=1`** is a load-test-only env hook (skips general + login limiters). Never set it in production.
